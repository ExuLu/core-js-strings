...
function getStringLength(value) {
  ...
  const stringLength = isValueString ? value.length : 0;
  return stringLength;
}
...
function isString(value) {
  ...
  if (typeof value === 'string') valueIsString = true;
  else if (typeof value === 'object' && value !== null) {
    const newValue = value.valueOf();
    valueIsString = typeof newValue === 'string';
  }
...
function repeatString(str, times) {
  const result = times > 0 ? str.repeat(times) : '';
  return result;
}
...
function removeFirstOccurrences(str, value) {
  ...
  const newString = str.slice(0, start).concat(str.slice(start + value.length));
  return start >= 0 ? newString : str;
}
...
function countVowels(str) {
  const regex = /[a, e, i, o, u, y, A, E, I, O, U, Y]/g;
...
function isPalindrome(str) {
  const newString = str.toLowerCase().replaceAll(/[^a-zA-Z0-9]/g, '');
...
function findLongestWord(sentence) {
  ...
  for (let i = 0; i < array.length; i += 1) {
    longestWord = array[i].length > longestWord.length ? array[i] : longestWord;
  }
  return longestWord;
}
...
function invertCase(str) {
  ...
  const upperArray = str.toUpperCase().split('');
  const resultArray = upperArray.map((letter, index) =>
    letter === array[index] ? letter.toLowerCase() : letter
  );
...
function encodeToRot13(str) {
  ...
  const cipheredArray = originalArray.map((letter) => {
    let newLetter = '';
    if (alphabet.includes(letter)) {
      const letterIndex = alphabet.indexOf(letter);
      newLetter = cipher.at(letterIndex);
